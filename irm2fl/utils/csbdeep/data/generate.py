...
def get_patch_indices(datas, patch_size, n_samples, datas_mask=None, patch_filter=None):
...
def sample_patches_from_multiple_stacks(datas, patch_size, n_samples,
    ...
    if not type(patch_size) is list:
        len(patch_size)==datas[1].ndim or _raise(ValueError())
        if not all(( 0 < s <= d for s,d in zip(patch_size,datas[1].shape) )):
    ...
    if sample_indices is None:
        rand_inds = get_patch_indices(datas, patch_size, n_samples, datas_mask=datas_mask, patch_filter=patch_filter)
    else:
        rand_inds = sample_indices

    if not type(patch_size) is list:
        patch_size = [patch_size] * len(datas)

    res = [np.stack([data[tuple(slice(0 if _r<0 else _r-(_p//2),_p if _r<0 else _r+_p-(_p//2)) \
                                for _r,_p in zip(r,patch_size[i]))] \
                                for r in zip(*rand_inds)]) \
                                for i, data in enumerate(datas) if not data is None]

    if datas[0] is None:
        return [None]*res[0].shape[0], res[0]
    else:
        return res
...
def create_patches(
        raw_data,
        patch_size, ### TODO / DL4IRM: integrate properly. In default function acts as intended.
        n_patches_per_image,
        patch_axes     = None,
        transforms     = None,
        patch_filter   = no_background_patches(),
        normalization  = norm_percentiles(), ### TODO / DL4IRM: integrate properly. In default function acts as intended.
        shuffle        = True,
        verbose        = True,
        sample_indices = None,  ### TODO / DL4IRM: integrate properly. In default function acts as intended.
        keep_channel   = False, ### TODO / DL4IRM: integrate properly. In default function acts as intended.
    ):
    ...
    sample_indices : list, optional
        list of indices for cropping of image patches
    ...
    if not sample_indices is None:
        assert len(sample_indices)==n_images, 'Length of images does not equal length of provided sample indices!'
        assert [len(i)==n_patches for i in sample_indices], "Each entry in 'sample_indices' must equal 'n_patches'!"
    ...
    X = np.empty((n_patches,)+tuple(patch_size[0]) if type(patch_size) is list else (n_patches,)+tuple(patch_size), dtype=np.float32)
    Y = np.empty((n_patches,)+tuple(patch_size[1]) if type(patch_size) is list else (n_patches,)+tuple(patch_size), dtype=np.float32)
    ...
    for i, (x,y,_axes,mask) in tqdm(enumerate(image_pairs),total=n_images,disable=(not verbose)):
        ...
        if i==0:
            axes = axes_check_and_normalize(_axes,len(patch_size[0] if type(patch_size) is list else patch_size))
        ...
        (_Y,_X) = sample_patches_from_multiple_stacks( (y,x), [i for i in reversed(patch_size)] if type(patch_size) is list else patch_size, n_patches_per_image, mask,
                                                       patch_filter, sample_indices=sample_indices if sample_indices is None else sample_indices[i] )

        s = slice(i*n_patches_per_image,(i+1)*n_patches_per_image)

        if type(normalization) is list:
            X[s] = normalization[0](_X, x)
            if not y is None:
                Y[s] = normalization[1](_Y, y)
            else:
    ...
    #### TODO / DL4IRM: integrate properly - this was enabled in original code!
    axes = 'S'+axes
    # axes = 'SC'+axes.replace('C','')
    # if channel is None:
    #     X = np.expand_dims(X,1)
    #     Y = np.expand_dims(Y,1) if not Y is None else Y
    # else:
    #     X = np.moveaxis(X, 1+channel, 1)
    #     Y = np.moveaxis(Y, 1+channel, 1) if not Y is None else Y
