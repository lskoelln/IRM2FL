...
class RawData(namedtuple('RawData' ,('generator' ,'size' ,'description'))):
    ...
    @staticmethod
    def from_files(basepath, source_files, target_files=None, axes='CZYX', pattern='*.tif*'):
        
        p = Path(basepath)

        if not target_files is None:
            pairs = [(p/source_files[n], p/target_files[n]) for n in range(len(source_files))]
        else:
            pairs = [(p/source_files[n], None) for n in range(len(source_files))]

        axes = axes_check_and_normalize(axes)
        n_images = len(pairs)
        description = "{p}: target={o}, sources={s}, axes='{a}', pattern='{pt}'".format(p=basepath, s='source files',
                                                                                        o='no target' if target_files is None \
                                                                                                      else 'target files',
                                                                                        a=axes, pt=pattern)
        ...
        def _gen():
            for fx, fy in pairs:
                x = imread(str(fx))
                y = None if target_files is None else imread(str(fy))  ### tifffile.imread reads file as: (C)YX
                if axes=='YXC' and x.ndim==3:
                    x = np.moveaxis(x,0,-1)
                if axes in ['CYX', 'YXC'] and x.ndim==2:
                    x = np.expand_dims(x, axis=-1 if axes=='YXC' else 0)
                if not y is None and axes == 'YXC' and y.ndim == 3:
                    y = np.moveaxis(y, 0, -1)
                if not y is None and axes in ['CYX', 'YXC'] and y.ndim==2:
                    y = np.expand_dims(y, axis=-1 if axes == 'YXC' else 0)
                len(axes) >= x.ndim or _raise(ValueError())
                yield x, y, axes[-x.ndim:], None
